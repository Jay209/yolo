...
def main():
    ...
    steps_per_epoch = len(trainset)  # if train images is 100 and batch size is 4 --> 25
    global_steps = tf.Variable(1, trainable=False, dtype=tf.int64)
    warmup_steps = TRAIN_WARMUP_EPOCHS * steps_per_epoch
    total_steps = TRAIN_EPOCHS * steps_per_epoch

    # if you want to do transfer training from already trained yolo weights, not from scratch
    if TRAIN_TRANSFER:
    ...
    # if your training fails somehow and want to continue training from where it stopped, this can be helpful.
    if TRAIN_FROM_CHECKPOINT:
        ...
        flag = True
        freezed_layers = ['conv2d', 'conv2d_1','conv2d_2','conv2d_3']
        for i, l in enumerate(yolo.layers):
          #  layer_weights = l.get_weights()

            if l.name in freezed_layers:
                l.trainable = False
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        if len(testset) == 0:
            ...
            yolo.save_weights(os.path.join(TRAIN_CHECKPOINTS_FOLDER, SAVED_MODEL_NAME))
            continue
        ...
        if TRAIN_SAVE_CHECKPOINT and not TRAIN_SAVE_BEST_ONLY:
            save_directory = os.path.join(TRAIN_CHECKPOINTS_FOLDER, SAVED_MODEL_NAME+"_val_loss_{:7.2f}".format(total_val/count))
            yolo.save_weights(save_directory)
        if TRAIN_SAVE_BEST_ONLY and best_val_loss>total_val/count:
            save_directory = os.path.join(TRAIN_CHECKPOINTS_FOLDER, SAVED_MODEL_NAME)
