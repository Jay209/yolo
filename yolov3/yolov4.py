...
def darknet19_tiny(input_data):
    input_data = convolutional(input_data, (3, 3, 3, 16))    # conv2d
    input_data = MaxPool2D(2, 2, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 16, 32))   # conv2d_1
    input_data = MaxPool2D(2, 2, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 32, 64))   # conv2d_2
    input_data = MaxPool2D(2, 2, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 64, 128))  # conv2d_3
    input_data = MaxPool2D(2, 2, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 128, 256)) # conv2d_4 26*26*256
    route_1 = input_data
    input_data = MaxPool2D(2, 2, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 256, 512)) # conv2d_5 13*13*512
    input_data = MaxPool2D(2, 1, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 512, 1024)) #conv2d_6 13*13*1024
...
def YOLOv3_tiny(input_layer, NUM_CLASS):
    ...
    conv = convolutional(conv, (1, 1, 1024, 256))   # conv2d_7  13*13*256
    conv_lobj_branch = convolutional(conv, (3, 3, 256, 512))   # conv2d_8 13*13*512
    
    # conv_lbbox is used to predict large-sized objects , Shape = [None, 13, 13, 255]
    conv_lbbox = convolutional(conv_lobj_branch, (1, 1, 512, 3*(NUM_CLASS + 5)), activate=False, bn=False)  #conv2d_9 13*13*18

    conv = convolutional(conv, (1, 1, 256, 128))     # conv2d_10 13*13*128
    # upsample here uses the nearest neighbor interpolation method, which has the advantage that the
    # upsampling process does not need to learn, thereby reducing the network parameter  
    conv = upsample(conv)
    
    conv = tf.concat([conv, route_1], axis=-1)   # 26*26*384
    conv_mobj_branch = convolutional(conv, (3, 3, 128, 256))  # conv2d_11  26*26*256
    # conv_mbbox is used to predict medium size objects, shape = [None, 26, 26, 255]
    conv_mbbox = convolutional(conv_mobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=False, bn=False)  #conv2d_12  26*26*18
