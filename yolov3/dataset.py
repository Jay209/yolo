...
class Dataset(object):
    ...
    def __init__(self, dataset_type, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
        ...
        self.num_samples = len(self.annotations)  # lenght of train.txt for 'train'
        self.num_batchs = int(np.ceil(self.num_samples / self.batch_size))   # if train images is 100 and batch size is 4 --> 25
        self.batch_count = 0
    ...
    def __next__(self):
        with tf.device('/cpu:0'):
            ...
            self.train_output_sizes = self.train_input_size // self.strides # 13 and 26 for yolo tiny

            batch_image = np.zeros((self.batch_size, self.train_input_size, self.train_input_size, 3), dtype=np.float32) # (batch_size, 416,416,3)

            if self.train_yolo_tiny:
                batch_label_mbbox = np.zeros((self.batch_size, self.train_output_sizes[0], self.train_output_sizes[0], self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32) # (batch_size, 26,26,3,5 + 1)
                batch_label_lbbox = np.zeros((self.batch_size, self.train_output_sizes[1], self.train_output_sizes[1], self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32) # (batch_size, 13,13,3,5 + 1)
            else:
            ...
            exceptions = False
            num = 0
            if self.batch_count < self.num_batchs:  # suppose total train images is 100 and batch size is 4 then total no of batches will be 25 that will be covered in 1 epoch
                while num < self.batch_size:   # this while loop will go until it coveres 4 images
                    index = self.batch_count * self.batch_size + num # 1st 4 images will be in 1st batch, next 4 will be in next batch .... will go on
                    if index >= self.num_samples: index -= self.num_samples
                    annotation = self.annotations[index]  # annotation of that selected image
                    image, bboxes = self.parse_annotation(annotation) # preprocessing(/255) and if augmentation is true, it will return 3 images from 1 image with corresponding bounding boxes
                    try:
                        if self.train_yolo_tiny:
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        for bbox in bboxes:
            ...
            deta = 0.01
            smooth_onehot = onehot * (1 - deta) + deta * uniform_distribution  # if class ind is 2 and #of class is 3 it will create vector like [0.09, 0.95,0.09]

            bbox_xywh = np.concatenate([(bbox_coor[2:] + bbox_coor[:2]) * 0.5, bbox_coor[2:] - bbox_coor[:2]], axis=-1) # convert xmin,ymin,xmax,ymax to xywh format
