...
def Load_Yolo_model():
    ...
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        ...
        if YOLO_CUSTOM_WEIGHTS == False:
            ...
            checkpoint = f"./checkpoints/{SAVED_MODEL_NAME}"
           # if TRAIN_YOLO_TINY:
            #    checkpoint += "_Tiny"
...
def draw_bbox(image, img_name, classification_model, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):
    #path = 'C:/Users/jj0276/Desktop/DL/YOLO-Object-Detection/classification/temp/'
    ...
    for i, bbox in enumerate(bboxes):
       ...
       # letter_image = image[y1:y2, x1:x2]
        # image_resized = cv2.resize(letter_image, (32, 32))
        # image_resized = np.array(image_resized)
        # image_resized = np.reshape(image_resized,(-1,32,32,3))
        # #cv2.imwrite(path + img_name + '_' +  str(i) + '.jpg', temp)
        # image_resized = image_resized / 255.0
        # pred = classification_model.predict(image_resized)
        # classes=np.argmax(pred,axis=1)
        # position = VAL_LIST.index(classes)
        ...
        if show_label:
            ...
            try:
                label = "{}".format(class_ind)  #class_name
            except KeyError:
...
def detect_image(Yolo, classification_model, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.15, iou_threshold=0.10, rectangle_colors=''):
    ...
    image = draw_bbox(original_image, img_name, classification_model, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
